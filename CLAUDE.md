...
- **`App.tsx` passes a nav variant as a prop** (`navModel: 'tabs' | 'top'`) to `HappyDayApp`. This renders fully different JSX branches for the nav chrome — check which variant you're editing, since `tabs`/`top` each have separate markup. (The home screen previously had a similar `homeLayout: 'poster' | 'cards'` switch; the unused `cards` branch was removed and `poster` is now the only home layout.)
- **Content is static data**, not fetched: [src/lib/data.ts](src/lib/data.ts) exports `VERSES`, `HYMNS`, `RECENT`, `HISTORY`, `TODAY`, `GROUP_MEMBERS`, `MEMBERS`. There's no backend — "today's passage," streaks, and history are hardcoded/derived client-side, not date-driven.
- **The only persisted state is the user's name and group**, read/written to `localStorage` under `happyday.name` / `happyday.group` (`readName`/`writeName`/`readGroup`/`writeGroup` in HappyDayApp.tsx), guarded with try/catch for private-mode/blocked storage. Sign-in (the "Me" tab when signed out) is a three-step flow — invitation code, then pick a group, then type a name (`signInStep: 'code' | 'group' | 'name'`). The invitation code is a single hardcoded value (`INVITE_CODE` in HappyDayApp.tsx) checked client-side only; there's no real auth or server round-trip. The name step's input shows an autocomplete dropdown of `GROUP_MEMBERS[draftGroup]` on focus (filtered as you type via `nameSuggestions`), but free typing is always allowed — it's not a constrained select.
...
- **[server/](server/)** is a separate Node.js + Express + TypeScript app (own `package.json`/`tsconfig.json`, not part of the Vite build) providing username/password auth backed by MySQL. Schema lives in [server/sql/schema.sql](server/sql/schema.sql) (`users`, `church_groups`, `daily_content`); [server/src/db/init.ts](server/src/db/init.ts) applies it (`npm run init-db` — the server no longer does this automatically on `npm run dev`/`start`) by naively splitting the file on `;` — **SQL comments in that file must never contain a literal `;`**, or `init.ts` will mis-split the file and corrupt the next statement (hit this once; MySQL CLI tools mask it since they parse the whole file properly). Routes: `POST /api/auth/register` and `POST /api/auth/login` in [server/src/routes/auth.ts](server/src/routes/auth.ts), returning a signed JWT. Configure via `server/.env` (see `.env.example`) — the frontend does not yet call this API; it's a standalone service.
- **`daily_content`** holds one row of devotional content (`passage`, `song`, `supplementary`) per day, assigned to exactly one of `user_id` or `group_id` (enforced by a `CHECK` constraint — MySQL 8.0.16+ required). A `church_groups` row must exist before content can target that group (FK), so `schema.sql` seeds the three groups from `src/App.tsx`'s `GROUPS` — keep those two lists in sync. Deleting a user or group cascades to their `daily_content` rows.
