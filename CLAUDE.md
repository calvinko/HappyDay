...
- **The only persisted state is the user's name**, read/written to `localStorage` under `happyday.name` (`readName`/`writeName` in HappyDayApp.tsx), guarded with try/catch for private-mode/blocked storage. 
Sign-in is just capturing a name; there's no auth.
