...
function HappyDayApp({ homeLayout = 'poster', navModel = 'tabs' }: Props) {
  ...
  return (
    <div className="flex min-h-svh items-center justify-center px-4 py-8">
      <div className="relative flex h-[812px] w-[390px] flex-col overflow-hidden border-2 border-ink bg-ground shadow-frame">
        <div className="flex flex-none items-center justify-between px-[18px] pt-[10px] pb-2 text-xs font-semibold tracking-wide">
          <span>9:41</span>
          <div className="flex items-center gap-1.5">
            <span className="block h-[9px] w-4 border-[1.5px] border-ink" />
            <span className="block h-[9px] w-[3px] bg-ink" />
          </div>
        </div>

        {navModel === 'top' && chrome && (
          <nav className="flex flex-none gap-5 border-b-2 border-ink px-[18px] pb-2.5">
            {(
              [
                ['Today', 'home'],
                ['Resources', 'resources'],
